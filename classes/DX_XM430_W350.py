...
class DX_XM430_W350:
    ...
    DRIVE_MODE = (
        10  # https://emanual.robotis.com/docs/en/dxl/x/xm430-w350/#drive-mode10
    )
    OPR_MODE = (
        11  # https://emanual.robotis.com/docs/en/dxl/x/xm430-w350/#operating-mode11
    )
    ...
    TICK_POS_OFFSET = (
        TICKS_PER_ROT / 2
    )  # position value for a joint angle of 0 (2048 for this case)
    ...
    def write_position(self, angle):
        position = (
            round(angle * self.TICKS_PER_DEG + self.TICK_POS_OFFSET)
            % self.TICKS_PER_ROT
        )
    ...
    def check_packet(self, addr, msg=None):
        dxl_comm_result = self.packet_handler.getLastTxRxResult(
            self.port_handler, self.PROTOCOL_VERSION
        )
        dxl_error = self.packet_handler.getLastRxPacketError(
            self.port_handler, self.PROTOCOL_VERSION
        )
        ...
        if dxl_comm_result != self.COMM_SUCCESS:
            print(
                f"[addr:{addr}] {self.packet_handler.getTxRxResult(self.PROTOCOL_VERSION, dxl_comm_result)}"
            )
            raise Exception("Communication Error: See above.")
        elif dxl_error != 0:
            print(
                f"[addr:{addr}] {self.packet_handler.getRxPacketError(self.PROTOCOL_VERSION, dxl_error)}"
            )
    ...
    def startConnection(self):
        if not self.portHandler.openPort():
            self._fatal(
                f"Failed to open serial port '{self.portHandler.getPortName()}'."
            )
        if not self.portHandler.setBaudRate(self.BAUDRATE):
            # Avoid leaving the port open on failure
            self.portHandler.closePort()
            self._fatal(
                f"Failed to set baud {self.BAUDRATE} on '{self.portHandler.getPortName()}'."
            )
    ...
    def enable_torque(self):
        self.packet_handler.write1ByteTxRx(
            self.port_handler, self.id, self.TORQUE_ENABLE, 1
        )

    def disable_torque(self):
        self.packet_handler.write1ByteTxRx(
            self.port_handler, self.id, self.TORQUE_ENABLE, 0
        )

    def write_data(self, address, data, length):
        if length == 1:
            dxl_comm, dxl_err = self.packet_handler.write1ByteTxRx(
                self.port_handler, self.id, address, data
            )
        elif length == 2:
            dxl_comm, dxl_err = self.packet_handler.write2ByteTxRx(
                self.port_handler, self.id, address, data
            )
        elif length == 4:
            # Important: write *TxRx* so we consume the status packet and avoid stale reads
            dxl_comm, dxl_err = self.packet_handler.write4ByteTxRx(
                self.port_handler, self.id, address, data
            )
        else:
    ...
    def read_data(self, address, length, retries: int = 3, delay: float = 0.005):
        ...
        for _ in range(retries):
            if length == 1:
                value, dxl_comm, dxl_err = self.packet_handler.read1ByteTxRx(
                    self.port_handler, self.id, address
                )
            elif length == 2:
                value, dxl_comm, dxl_err = self.packet_handler.read2ByteTxRx(
                    self.port_handler, self.id, address
                )
            elif length == 4:
                value, dxl_comm, dxl_err = self.packet_handler.read4ByteTxRx(
                    self.port_handler, self.id, address
                )
            else:
                return 0

...
class OM_X_arm:
    ...
    def __init__(self):
        ...
        self.gripperID = 15
        self.deviceName = '/dev/tty.usbserial-FT94EOVP' # self.find_device_name()
        print(f"Device name: {self.deviceName}")
        ...
        self.motors = [
            DX_XM430_W350(
                self.port_handler, self.packet_handler, self.deviceName, motor_id
            )
            for motor_id in self.motorIDs
        ]

        # Create Gripper and set operating mode/torque
        self.gripper = DX_XM430_W350(
            self.port_handler, self.packet_handler, self.deviceName, self.gripperID
        )
        ...
        if not self.port_handler.setBaudRate(DX_XM430_W350.BAUDRATE):
            ...
            self._fatal(
                f"Could not set {DX_XM430_W350.BAUDRATE} on '{self.port_handler.getPortName()}'"
            )
        ...
        self.bulk_read_write(
            ...
            [enable] * self.motorsNum,
        )
        self.bulk_read_write(
            DX_XM430_W350.DRIVE_MODE_LEN,
            DX_XM430_W350.DRIVE_MODE,
            [DX_XM430_W350.TIME_PROF] * self.motorsNum,
        )
    ...
    def find_device_name(self):
        ...
        def is_tty(dev):
            return dev.startswith(
                ("/dev/ttyUSB", "/dev/ttyACM", "/dev/rfcomm", "/dev/tty.")
            )

        def looks_ftdi(p):
            s = " ".join(
                str(getattr(p, k, "") or "")
                for k in ("manufacturer", "description", "product", "interface")
            ).lower()
            return (
                getattr(p, "vid", None) == 0x0403
                or any(
                    t in s
                    for t in (
                        "ftdi",
                ...
                or ("uart" in s and "ft" in s)
            )

        ftdi, other = [], []
        for p in ports:
            dev = p.device
            if is_tty(dev) or ("COM" in dev and dev != "COM1"):
                (ftdi if looks_ftdi(p) else other).append(dev)

        for lst in (ftdi, other):
            if lst:
                return sorted(lst)[0]
        self._print_port_hint()
        ...
        if msgs is None:  # Bulk read
            ...
            for motor_id in self.motorIDs:
                ...
                if not ok:
                    self.groupBulkRead.clearParam()
                    raise RuntimeError(
                        f"[ID:{motor_id:03d}] GroupBulkRead addParam failed (addr=0x{addr:X}, n={n})"
                    )
            ...
            for motor_id in self.motorIDs:
                if not self.groupBulkRead.isAvailable(motor_id, addr, n):
                    self.groupBulkRead.clearParam()
                    raise RuntimeError(
                        f"[ID:{motor_id:03d}] BulkRead data not available at 0x{addr:X} (len={n})"
                    )
            ...
            if len(msgs) != len(self.motorIDs):
                raise ValueError(
                    f"'msgs' length {len(msgs)} must match number of motors {len(self.motorIDs)}"
                )
            ...
            for i, motor_id in enumerate(self.motorIDs):
                ...
                if not ok:
                    ...
                    raise RuntimeError(
                        f"[ID:{motor_id:03d}] GroupBulkWrite addParam failed (addr=0x{addr:X}, n={n})"
                    )

    ...
    try:
        while True:
            ...
            if len(tags) > 0:
                ...
                # Use np.linalg.pinv()
                # YOUR CODE HERE
                qdot_rad = (np.linalg.pinv(J_linear) @ velocity_cmd.reshape(3, 1)).reshape(4)  # rad/s
                ...
                data['pos_current'].append(current_ee_pos.tolist())
                data['pos_desired'].append(desired_ee_pos.tolist())
                data['error'].append(error.tolist())
                data['control_output'].append(velocity_cmd.tolist())
                data['joint_vel'].append(qdot_rad.tolist())     # rad/s as requested
                data['tag_detected'].append(True)
                # --------------------------------------------------------------------
        ...
        # -------------------------- SAVE LOGGED DATA --------------------------
        try:
            with open('visual_servoing_data.pkl', 'wb') as f:
                pickle.dump(data, f)
            print("Saved log to visual_servoing_data.pkl")
        except Exception as e:
